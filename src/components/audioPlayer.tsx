    ...
    <React.Fragment>
      {ready && (
        <div className="w-full m-2 flex flex-col items-center">
          <p className="text-white">{songsMetadata[currentSong]?.title}</p>
          <p className="text-white italic">
            {songsMetadata[currentSong]?.artist}
          </p>
          <p className="text-white font-semibold">
            {songsMetadata[currentSong]?.album}
          </p>
          <div className="w-full grid grid-cols-3 justify-center text-white p-2 ">
            <span />
            <div className="w-full items-center justify-center flex gap-5">
              ...
              <button aria-label="next song" type="button" onClick={handleNext}>
                <FaStepForward />
              </button>
              <button aria-label="loop" type="button" onClick={handleLoop}>
                <RxLoop />
              </button>
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={() => setMuted((prev) => !prev)}>
                {muted ? <FaVolumeMute /> : <RxSpeakerModerate />}
              </button>
            </div>
          </div>
          <TimeLabel />
